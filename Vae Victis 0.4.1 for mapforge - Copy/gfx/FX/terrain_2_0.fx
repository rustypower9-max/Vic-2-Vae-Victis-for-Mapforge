...
float4 PixelShader_Map2_0_General_Low(VS_MAP_OUTPUT v) : COLOR
{
	...
	float2 texSize = float2(ColorMapWidth, ColorMapHeight);
    float2 texelSize = 1.0 / texSize;
	
	float borderFactor = 0; // 0
    int sampleCount = 0;
    int sampleRadius = 16; // Reduced sampling radius from 12
    ...
    //        colorDiff = abs(sampleColor - TargetRGB);
	...
	float3 parchmentColor = float3(232.0 / 255.0, 224.0 / 255.0, 184.0 / 255.0);
	float3 parchmentColor2 = float3(232.0 / 255.0, 224.0 / 255.0, 184.0 / 255.0);
	...
	float4 TerrainColorX = float4(grey, grey, grey, TerrainColor.a);
	TerrainColorX.rgb = lerp(TerrainColorX.rgb, parchmentColor2, 0.65);
    float4 FinalColor = lerp(MetaColor, TerrainColorX, ColorMatch);
