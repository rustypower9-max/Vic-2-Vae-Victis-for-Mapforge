...
float4 PixelShader_Far(VS_OUTPUT_WATER_FAR IN) : COLOR
{
    ...
    float3 deepWaterColor = float3(232.0 / 255.0, 224.0 / 255.0, 184.0 / 255.0);  // This is actualy coastal water 
    ...
    for (int x = -sampleRadius; x <= sampleRadius; x += 4) {
        for (int y = -sampleRadius; y <= sampleRadius; y += 4) {
	...
	float grey = (baseColor.r + baseColor.g + baseColor.b) / 3.0;
	float4 baseColorX = float4(grey, grey, grey, baseColor.a);
    ...
    float3 newWaterColor = lerp(baseColorX.rgb, parchmentColor, 0.75); // 0.75
